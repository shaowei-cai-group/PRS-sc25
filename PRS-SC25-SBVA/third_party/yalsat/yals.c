...
typedef struct Callbacks {
  ...
  struct { void * state; void (*fun)(void*); } bestphase; // 新增 bestphase 回调
} Callbacks;
...
static void yals_save_new_minimum (Yals * yals) {
  ...
  // 调用 best phase 回调函数
  if (yals->cbs.bestphase.fun) {
    yals->cbs.bestphase.fun(yals->cbs.bestphase.state);
  }
...
static void yals_init_weight_to_score_table (Yals * yals) {
  ...
  if (yals->opts.cb.val >= 100)
    cb = yals->opts.cb.val/100.0;

  // probSAT SC'13 values:

  else if (maxlen <= 3) cb = 2.5;	// from Adrian's thesis ...
...
static void yals_connect (Yals * yals) {
  ...
  if (minlen == maxlen) uniform = !yals->opts.toggleuniform.val;
  else uniform = yals->opts.toggleuniform.val;
