...
inline char *read_whitespace(char *p) {
    while ((*p >= 9 && *p <= 13) || *p == 32)
        ++p;
    return p;
}
inline char *read_until_new_line(char *p)
{
    while (*p != '\n')
    {
        if (*p == '\0')
...
inline char *read_int(char *p, int *i)
{
    *i = 0;
    bool sym = true;
    p = read_whitespace(p);
...
inline void readfile(const char *file, int *vars, int *clauses, vec<vec<int>> &clause) {
