...
static bool sweep_backbone_candidate (sweeper *sweeper, unsigned lit) {
  ...
  if (res == 20) {
    ...
    // printf("c sweep backbone %d\n", lit);

    if(solver->prsExportState != NULL) {
      // share sweep backbone
      int elit = kissat_export_literal(solver, lit);
      cvec_clear(solver->exportedClause);
      cvec_push(solver->exportedClause, elit);
      solver->prsExportClause(solver->prsExportState, solver->exportedClause, 1);
    }
      ...
      if (head.type.binary) {
        ...
        for (all_literals_in_clause (other, c)) {
          if (other == lit) {
            ...
            found = true;
#endif
            PUSH_STACK (solver->clause.lits, repr);
            continue;
          }
        ...
        if (new_size == 1) {
          ...
          const unsigned unit = POP_STACK (solver->clause.lits);
          CHECK_AND_ADD_UNIT (unit);
          ADD_UNIT_TO_PROOF (unit);
          kissat_assign_unit (solver, unit);
          INC (sweep_units);
        ...
        if (!c->redundant)
          kissat_mark_added_literals (solver, new_size,
                                      BEGIN_STACK (solver->clause.lits));

        if (new_size == 2) {
          const unsigned second = POP_STACK (solver->clause.lits);
          const unsigned first = POP_STACK (solver->clause.lits);
          ...
          watch dst = kissat_binary_watch (repr, true, 0);
          watches *other_watches = &WATCHES (other);
          kissat_substitute_large_watch (solver, other_watches, src, dst);
        ...
        const unsigned *const begin = BEGIN_STACK (solver->clause.lits);
        const unsigned *const end = END_STACK (solver->clause.lits);
...
static bool sweep_equivalence_candidates (sweeper *sweeper, unsigned lit,
  ...
  if (res == 10) {
    ...
    // printf("c unkown equivalence lit %d other %d\n", lit, other);
  ...
  // printf("c sweep equivalence %d = %d\n", lit, other);

  // share equivalence

  if(solver->prsExportState != NULL) {
    int elit = kissat_export_literal(solver, lit);
    int elit_not = kissat_export_literal(solver, not_lit);
    
    int elit_other = kissat_export_literal(solver, other);
    int elit_not_other = kissat_export_literal(solver, not_other);

    cvec_clear(solver->exportedClause);
    cvec_push(solver->exportedClause, elit);
    cvec_push(solver->exportedClause, elit_not_other);

    solver->prsExportClause(solver->prsExportState, solver->exportedClause, 1);

    cvec_clear(solver->exportedClause);
    cvec_push(solver->exportedClause, elit_not);
    cvec_push(solver->exportedClause, elit_other);
    solver->prsExportClause(solver->prsExportState, solver->exportedClause, 1);
  }
...
DONE:
  ...
  if (!solver->inconsistent && !kissat_propagated (solver))
    (void) kissat_dense_propagate (solver, 2e9, -1);
...
static unsigned schedule_all_other_not_scheduled_yet (sweeper *sweeper) {
  ...
  RADIX_STACK (8, sweep_candidate, unsigned, fresh, RANK_SWEEP_CANDIDATE);
...
bool kissat_sweep (kissat *solver) {
  ...
  // if (!solver->inconsistent) {
  //   solver->propagated = solver->trail.begin;
  //   kissat_probing_propagate (solver, 0);
  // }
